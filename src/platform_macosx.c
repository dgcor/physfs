...
/* Wrap PHYSFS_Allocator in a CFphys_fs_allocator... */
...
{
    return physfs_alloc.Malloc(allocSize);
} /* cfallocMalloc */


static void cfallocFree(void *ptr, void *info)
{
    physfs_alloc.Free(ptr);
} /* cfallocFree */
                            ...
                            CFOptionFlags hint, void *info)
{
    ...
    return physfs_alloc.Realloc(ptr, newsize);
} /* cfallocRealloc */
...
static char *convertCFString(CFStringRef cfstr)
{
    ...
    char *retval = (char *) physfs_alloc.Malloc(len);
    ...
    if (CFStringGetCString(cfstr, retval, len, kCFStringEncodingUTF8))
    {
        ...
        if (newlen < len)
        {
            void *ptr = physfs_alloc.Realloc(retval, newlen);
            if (ptr != NULL)
                retval = (char *) ptr;
        } /* if */
    } /* if */
...
{
    physfs_alloc.Init = macosxAllocatorInit;
    physfs_alloc.Deinit = macosxAllocatorDeinit;
    physfs_alloc.Malloc = macosxAllocatorMalloc;
    physfs_alloc.Realloc = macosxAllocatorRealloc;
    physfs_alloc.Free = macosxAllocatorFree;
    return 1;  /* return non-zero: we're supplying custom physfs_alloc. */
