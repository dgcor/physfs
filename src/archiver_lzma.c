...
{
    return ((size == 0) ? NULL : physfs_alloc.Malloc(size));
} /* SzAllocPhysicsFS */
...
{
    /* Free arrays */
    physfs_alloc.Free(archive->folders);
    physfs_alloc.Free(archive->files);
    physfs_alloc.Free(archive);
}
...
static void LZMA_destroy(PHYSFS_Io *io)
{
    ...
    if (file->folder != NULL)
    {
        ...
        if (file->folder->references == 0)
        {
            ...
            physfs_alloc.Free(file->folder->cache);
...
static void *LZMA_openArchive(PHYSFS_Io *io, const char *name, int forWriting)
{
    ...
    archive = (LZMAarchive *) physfs_alloc.Malloc(sizeof (LZMAarchive));
    ...
    archive->files = (LZMAfile *) physfs_alloc.Malloc(len);
    ...
    archive->folders = (LZMAfolder *) physfs_alloc.Malloc(len);
