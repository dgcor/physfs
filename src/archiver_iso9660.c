...
static void *ISO9660_openArchive(PHYSFS_Io *io, const char *filename, int forWriting)
{
    ...
    handle = physfs_alloc.Malloc(sizeof(ISO9660Handle));
    GOTO_IF_MACRO(!handle, PHYSFS_ERR_OUT_OF_MEMORY, errorcleanup);
    handle->path = 0;
    handle->mutex= 0;
    handle->io = NULL;

    handle->path = physfs_alloc.Malloc(strlen(filename) + 1);
...
{
    physfs_alloc.Free(fhandle->cacheddata);
    physfs_alloc.Free(fhandle);
} /* iso_file_close_mem */
...
{
    fhandle->cacheddata = physfs_alloc.Malloc(fhandle->filesize);
...
static PHYSFS_Io *ISO9660_openRead(void *opaque, const char *filename)
{
    ...
    fhandle = physfs_alloc.Malloc(sizeof(ISO9660FileHandle));
    BAIL_IF_MACRO(fhandle == 0, PHYSFS_ERR_OUT_OF_MEMORY, NULL);
    fhandle->cacheddata = 0;

    retval = physfs_alloc.Malloc(sizeof(PHYSFS_Io));
...
errorhandling:
    if (retval) physfs_alloc.Free(retval);
    if (fhandle) physfs_alloc.Free(fhandle);
    return NULL;
} /* ISO9660_openRead */
